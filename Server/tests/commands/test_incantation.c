...
static linked_list_t init_players(void)
{
    ...
    *player = (player_t){3, 1, 2, 4, {0}, 3, 3, 4, 0, 0, 0};
    append(&list, player);
    return (list);
}
...
Test(send_incantation_start, simple_test)
{
    ...
    player_t plyr = {3, 1, 2, 4, {0}, 3, 3, 4, 0, 0, 0};
    ...
    cmd[PLAYER_SENDER_POS] = (cmd_info_t){&plyr, sizeof(player_t)};
    srv.players[0] = 1;
    cr_assert_neq(fd, -1);
...
Test(send_incantation_end, simple_test)
{
    ...
    cmd[CUSTOM_SENDER_POS] = (cmd_info_t){&srv, sizeof(srv)};
    cr_assert_neq(fd, -1);
